...
from models import groups, tests, mdiagfile
...
class Euphonia():
    ...
    def editGroupNote(self, sfid, text, **kwargs):
        ...
        res = self.sfdc._editSFNote(sfid, text)
        if not res['ok']:
            return res
    ...
    def deleteGroupNote(self, sfid, **kwargs):
        ...
        res = self.sfdc._deleteSFNote(sfid)
        if not res['ok']:
            return res
    ...
    def start(self):
        ...
        def issue_summary(**kwargs):
            ...
            return bottle.template(
                ...
                allWorkflows=workflowMap, content=content)
        ...
        @tokenize
        def add_to_set(wfname, setname, **kwargs):
            return self.kk.workflowRequest(wfname, "addtoset", setname,
                                           **kwargs)

        @b.route('/workflow/<wfname>/rmfromset/<setname>')
        @tokenize
        def rm_from_set(wfname, setname, **kwargs):
            return self.kk.workflowRequest(wfname, "rmfromset", setname,
                                           **kwargs)
        ...
        @b.route('/workflows/actions')
        @tokenize
        def workflows_actions(**kwargs):
            import inspect
            from bson import json_util
            from jirapp import jirapp

            tests = {}
            sources = {"jira":jirapp,"salesforce":sfdcpp}
            for source, classObj in sources.iteritems():
                tests[source] = {}
                functions = inspect.getmembers(classObj, predicate=inspect.ismethod)
                for key, value in functions:
                    if key.startswith("_") == False:
                        argList = []
                        for arg in inspect.getargspec(value)[0]:
                            if arg != None and arg != "self" and arg != "key" and arg != "companyId":
                                argList.append(arg)
                        tests[source][key] = argList
            return json_util.dumps(tests)

        # UPLOAD-RELATED ROUTES
        @b.route('/mdiag')
        def mdiag_form(**kwargs):
            return bottle.template('base_page', renderpage="mdiag")

        @b.route('/mdiag/ticketinfo/<jira_key>')
        def mdiag_ticketinfo(jira_key, **kwargs):
            md = mdiagfile.MdiagFile(self.mongo)
            results = md.getTicketInfo(jira_key)
            return bson.json_util.dumps(results)

        @b.post('/mdiag/upload')
        def mdiag_upload_files(**kwargs):
            ...
            md = mdiagfile.MdiagFile(self.mongo)
            company = md.getTicketInfo(jira_key).get('company')
            isSaved = md.upload(content, jira_key)
            if isSaved is True:
                gmd = groupmdiag.GroupMdiag(company, mongo=self.mongo)
                definitions = gmd.getTestDefinitions()
                results = gmd.run_all_tests()
                for test in definitions:
                    if test in results and 'payload' in results[test]:
                        definitions[test]['pass'] = results[test]['payload']['pass']
            return bson.json_util.dumps({'res': 'ok', 'payload': definitions})
