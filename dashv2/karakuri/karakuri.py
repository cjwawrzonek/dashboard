...
class karakuri:
    ...
    def buildValidateQuery(self, workflowNameORworkflow, _id=None, task=None,
        ...
        # do not include docs for which the given workflow has already been
        # performed!
        # TODO this is not what we want to do in the end, why not
        # run the same workflow twice?
        # perhaps we need a new concept, workflow chains, to mark the end of
        # the line
        if "$and" not in match:
            match["$and"] = []
    ...
    def validateTask(self, tid, **kwargs):
        ...
        try:
            res = self.mongo[db][coll].find(match).count() != 0
        except pymongo.errors.PyMongoError as e:
            self.logger.exception(e)
            return {'ok': False, 'payload': e}
