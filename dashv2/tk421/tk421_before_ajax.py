...
class tk421():
    ...
    def getIssues(self, data):
        ...
        #         updated_data["ACTS"].append(self.trimmedACTSIssue(issue))      
    ...
    def mergeAndSortIssues(self, dbd_issues, updated_issues):
        ...
        sorters = {
            ...
            "UNA": descendingTimeOrder, "ACTS": descendingTimeOrder
        }
    ...
    def start(self):
        ...
        # os.chdir('/Users/cjw/dashv2/tk421')
        ...
        def ajax_response():
            ...
            logger.info("-- time before running getIssues(): {}".format(dt))

            # logger.info('Here is gData passed to the server: ' + str(bottle.request.json))

            try:
                if (bottle.request.json):# and 'totals' in bottle.request.json):
                    issues = self.getIssues(bottle.request.json)
            ...
            logger.info("-- time to run through issues using getIssues(): {} --".format(dt2))
            # cjwlog.info("Time to run through issues using getIssues(): {}".format(dt2))

            return bson.json_util.dumps(issues)
