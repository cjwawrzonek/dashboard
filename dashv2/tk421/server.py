...
#     logfile.write("Time to make initial connection through MongoClient(): ")
#     print >> logfile, dt

#     try:
#         if (request.json and 'totals' in request.json):
#             issues = getIssues(support_db, request.json, start, logfile)
#         else:
#             issues = getIssues(support_db, {}, start, logfile)
...
#     logfile.write("Time to run through issues using getIssues(): ")
#     print >> logfile, dt2
#     logfile.close()
...
def ajax_response():
    ...
    dt = int((time.time() - start)*1000)
    # cjwlog.info("Time to make initial connection through MongoClient(): {}".format(dt))
    ...
    dt2 = int((time.time() - start)*1000)
    # cjwlog.info("Time to run through issues using getIssues(): {}".format(dt2))
...
# def getActiveSLAs(db, data, stime, logfile):

# def querySLAs(db, data):
...
#                     'Waiting For User Input']}  #, 'Waiting for Customer'
#             }
#         )
#     else:
...
def getIssues(db, data, stime, logfile):
    ...
    dt = int((time.time() - stime)*1000)
    logfile.write("Time to before loop: ")
    print >> logfile, dt   

    issues_list = [i for i in cur]

    dt = int((time.time() - stime)*1000)
    logfile.write("Time to after data fetch: ")
    print >> logfile, dt   
    ...
    for i in issues_list:
        ...
        dt = int((time.time() - stime)*1000)
        logfile.write("Time to loop start: ")
        print >> logfile, dt   

        issue = SupportIssue().fromDoc(i)

        dt = int((time.time() - stime)*1000)
        logfile.write("Time to after SupportIssue().fromDoc(i): ")
        print >> logfile, dt

        # dt = int((time.time() - stime)*1000)
        # logfile.write("First run in loop: ")
        # print >> logfile, dt
    ...
    dt = int((time.time() - stime)*1000)
    logfile.write("Time to after for loop: ")
    print >> logfile, dt

    mergeAndSortIssues(dbd_data, updated_data)

    dt = int((time.time() - stime)*1000)
    logfile.write("Time to after mergeAndSortIssues() run: ")
    print >> logfile, dt
...
if __name__ == "__main__":
    ...
    # cjwlog = logging.getLogger("cjwlog")
    # fileHr = logging.FileHandler("/Users/cjw/dashv2/tk421/cjwlog.log", "a")
    # cjwlog.setLevel(logging._levelNames[args.log_level])
    # fileHr.setFormatter(formatter)
    # cjwlog.addHandler(fileHr)
