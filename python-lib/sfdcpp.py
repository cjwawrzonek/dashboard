...
class sfdcpp():
    ...
    def _getAccounts(self):
        ...
        return self._querySFDC(query)

    # First_MMS_Date__c, Fiscal_Year_End__c,
    # Number_of_Emails_and_Email_Domains__c, Rankings_DandB_Import__c,Site
    ...
    def _getAccountOpportunities(self, accountId):
        self.logger.debug("getAccountOpportunities(%s)", accountId)
        query = """SELECT Id FROM Opportunity WHERE AccountId = '%s'""" %\
                accountId
        return self._querySFDC(query)

    def _getAccountOpportunity(self, opportunityId):
        self.logger.debug("getAccountOpportunity(%s)", opportunityId)
        query = """SELECT AccountId,ACV_Bookings_Net_New__c,
                   ACV_Bookings_Renewal__c,ACV_Bookings__c,
    ...
    # Account_Region_Intacct_Id__c, ACV_Include__c, Delivered_Amt__c, Discount,
    # Finance_Bucket__c, Intacct_Location_ID__c, Make_whole_for_Commissions__c,
    # Make_whole_for_Finance__c, Opp_Owner_Intacct_Id__c,
    # Product_Sub_Family__c, Renewable__c, ServiceEndDate__c, Subtotal,
    # Suppress_Commissions__c, Term_Days_Default__c
    def _getAccountOpportunityProducts(self, opportunityId):
        ...
        query = """SELECT Active_Subscriptions__c,ACV_Line__c,
                   ...
                   FROM OpportunityLineItem WHERE OpportunityId = '%s'""" %\
            opportunityId
        return self._querySFDC(query)

    # Initial_Response_Entitlement__c, Support_Level__c,
    def _getAccountProjects(self, accountId):
    ...
    def _getListOfAccountIds(self):
        self.logger.debug("getListOfAccountIds()")
        res = self._getAccounts()
        if not res['ok']:
            return res
    ...
    def _run(self):
        ...
        for sid in self.args['sid']:
            ...
            if self.args['query_type'] == "all":
                res = self._saveAccount(sid, details=True)
                if not res['ok']:
                    self.logger.warning(res['payload'])
            elif self.args['query_type'] == "accountOnly":
                res = self._saveAccount(sid, details=False)
                if not res['ok']:
                    self.logger.warning(res['payload'])
            else:
    ...
    def _saveAccount(self, accountId, details=True):
        self.logger.debug("saveAccount(%s,%s)", accountId, details)
        res = self._getAccount(accountId)
        if not res['ok']:
            self.logger.warning(res['payload'])
            return res
        ...
        doc = res['payload'][0]
        if details is True:
            res = self._getAccountOpportunities(accountId)
            if not res['ok']:
                self.logger.warning(res['payload'])
                return res
            ...
            for opportunity in opportunities:
                ...
                res = self._getAccountOpportunityProducts(oppId)
                if not res['ok']:
                    self.logger.warning(res['payload'])
            ...
            res = self._getAccountProjects(accountId)
            if not res['ok']:
                self.logger.warning(res['payload'])
                return res
            doc['projects'] = res['payload']

        doc = self._convertFieldIllegals(doc)
        doc['_id'] = accountId
    ...
    def addSFPublicAccountNote(self, companyId, title, comment):
        ...
        if not res['ok']:
            return res
        sfId = res['payload']
        return self._addSFNote(sfId, title, comment, False)
    ...
    def addSFPrivateAccountNote(self, companyId, title, comment):
        ...
        if not res['ok']:
            return res
        sfId = res['payload']
        return self._addSFNote(sfId, title, comment, True)
    ...
    def _addSFNote(self, sfId, title, comment, isPrivate):
    ...
    def _editSFNote(self, sfId, comment):
