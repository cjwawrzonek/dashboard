...
class SupportIssue:
    ...
    def isProactive(self):
        return (self.issuetype.lower() == 'proactive' and
    ...
    @property
    def assigneeDisplayName(self):
        if self.hasJIRA():
            if 'assignee' in self.doc['jira']['fields'] and\
                    self.doc['jira']['fields']['assignee']:
                return self.doc['jira']['fields']['assignee']['displayName']
        return None
    ...
    def reporterEmail(self):
        if self.hasJIRA():
            if 'reporter' in self.doc['jira']['fields'] and\
                ...
                return self.doc['jira']['fields']['reporter']['emailAddress']
        return None
    ...
    def isNew(self):
        return self.lastXGenPublicComment is None

    def isNewProactiveIssue(self):
        return (self.doc['dash']['active']['now'] and
                self.isProactive() and
                self.firstCustomerComment is None)

    def isActive(self):
        if self.isNewProactiveIssue():
            return False

        return (self.doc['dash']['active']['now'] and
                self.status in ('Open', 'Reopened', 'In Progress'))

    def isWFC(self):
        if self.isNewProactiveIssue():
            return True
        # regardless of status, if the SLA has not been met the ticket is
        # active
        elif self.isNew():
            return False
        return (self.doc['dash']['active']['now'] and
                self.status in ('Waiting for Customer',
